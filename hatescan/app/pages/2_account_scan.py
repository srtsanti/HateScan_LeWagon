...
def account_scan_page():
    ...
    st.write('Find out how hateful an Account is and what topic they tweet about the most')
    ...
    st.subheader("Scan a Twitter account")
    user = st.text_input("Enter the username (without the @) :", max_chars=50)
    ...
    if scanner_user:
        if not query_result.empty:
            ...
            followers = query_result['nr_followers'].item()
            ...
            with col3:
                if scale in scale_mapping:
                    ...
                    st.metric("Account Hate Label",f"{label} {emoji}")
            with col4:
                st.metric('Number of Followers', '{:,}'.format(followers))
            ...
            api_fig_topics = px.bar(dfr, x='variable', y='value', color='variable', title=f"Topic's {user} tweets about:", color_discrete_sequence=color_palette)
